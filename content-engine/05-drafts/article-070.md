...
slug: sales-tax-nexus-small-online-seller
...
primary_keyword: "sales tax nexus small online seller"
secondary_keywords: ["economic nexus vs physical nexus", "do i need to collect sales tax in every state", "sales tax nexus states list", "amazon fba sales tax nexus"]
intent: Problem-solving
audience: "Small US-based ecommerce sellers (Etsy/Shopify/Amazon FBA) and small SaaS founders trying to figure out where they legally owe sales tax"
topic_cluster: "us-sales-tax-compliance"
countries: ["United States"]
author: "SCULT.IN Content Engine"
review_status: draft
last_verified: "2026-08-21"
sources: ["https://www.taxjar.com/sales-tax/nexus", "https://hn.algolia.com/api/v1/search?query=sales%20tax%20nexus", "https://taxcloud.com/blog/sales-tax-nexus-by-state/", "https://www.numeral.com/blog/economic-nexus", "https://nexusbystate.com/"]
---

# What "Sales Tax Nexus" Actually Means for a Small Online Seller
...
- What sales tax nexus actually is
- Physical nexus: what creates it
- Economic nexus: the Wayfair-era rule
- Current 2026 state thresholds
- Amazon FBA and marketplace-specific nexus issues
- Do marketplaces like Shopify or Etsy collect tax for you automatically?
- Practical examples
- Data and evidence
- Comparisons
...
Sales tax nexus is the legal term for the connection between a business and a state that's substantial enough to obligate the business to register with that state, collect sales tax from customers there, and remit it to the state's tax authority ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus)). It's a foundational concept because everything downstream — whether you need to register, whether you need to charge tax on an invoice, whether you owe back taxes for past sales — depends on first establishing whether nexus exists in a given state.

Critically, nexus is evaluated **state by state, independently** — there is no single national nexus test. Each state sets its own rules, its own thresholds, and its own definitions of what counts, which means a seller can have nexus in a handful of states and none in others, and the specific combination is unique to that seller's actual footprint and sales pattern ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus)).
...
- **An office or place of business** located in the state.
- **Employees or staff** working in the state — including remote employees, which is a real and actively discussed concern among small SaaS founders specifically, per a dedicated "Ask HN: Hiring Remotely and US Tax Nexuses" discussion thread ([hn.algolia.com](https://hn.algolia.com/api/v1/search?query=sales%20tax%20nexus)).
- **Inventory stored in the state** — explicitly including inventory held in third-party fulfillment centers, such as an Amazon FBA warehouse ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus)).
- **Temporary physical presence**, such as attending a trade show or craft fair in the state.
- **Third-party affiliates** operating on your behalf in the state.
...
Economic nexus is the newer, and for most online-only sellers, more relevant form of nexus — and it has no physical-presence requirement at all. It's triggered purely by crossing a state-set threshold of sales volume, sales dollar amount, or transaction count into that state, regardless of whether you have any physical footprint there whatsoever ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus)).
...
- **46 states** publish a sales-tax economic nexus dollar threshold for remote sellers (the remainder either have no general sales tax or handle nexus differently).
- **41 of those states** use the common **$100,000** in sales as the dollar threshold.
- **2 states use $250,000** (Alabama and Mississippi).
- **3 states use $500,000** (California, Texas, and New York).
- **18 jurisdictions** still maintain an additional **200-transaction test** alongside (or as an alternative to) the dollar threshold — meaning a seller with many small, low-average-order-value transactions can trip economic nexus in these states well before reaching six figures in actual revenue.
- Some states are actively **removing** the transaction-count test: **Alaska** repealed its 200-transaction threshold effective January 1, 2025, and **Illinois** removed its 200-transaction threshold effective January 1, 2026 — both moving toward dollar-threshold-only nexus rules.
- **New York and Connecticut** use a combined **"AND" test** rather than an "or" test: New York requires **both** $500,000 in sales **and** 100 transactions before nexus triggers; Connecticut requires **both** $100,000 **and** 200 transactions ([taxcloud.com](https://taxcloud.com/blog/sales-tax-nexus-by-state/); [numeral.com](https://www.numeral.com/blog/economic-nexus); [nexusbystate.com](https://nexusbystate.com/)).
...
Beyond the general physical-nexus-via-warehoused-inventory issue described above, Amazon FBA sellers face an additional layer of complexity worth being aware of: many states have since passed **marketplace facilitator laws**, which shift the actual tax *collection and remittance* obligation onto the marketplace itself (Amazon, Etsy, etc.) for sales made through that marketplace, rather than requiring the individual seller to collect and remit directly for those specific transactions. This doesn't eliminate nexus considerations entirely — a seller can still have nexus-relevant obligations for sales made outside the marketplace (their own website, other channels) even while the marketplace handles collection for in-marketplace sales — but it does meaningfully change the practical, day-to-day compliance burden compared to the pre-marketplace-facilitator-law landscape that existed when FBA-related nexus concerns first became a major small-seller pain point.

## Do marketplaces like Shopify or Etsy collect tax for you automatically?

This is a genuinely common point of confusion, and the honest answer is: **it depends on the platform, the specific transaction, and increasingly, on marketplace facilitator laws — not something a seller should assume without checking.** Shopify, as a platform you host your own store on, provides tax calculation and collection tools, but the underlying responsibility for correctly configuring nexus-based tax collection across the states where you actually have nexus generally remains with the seller, unlike a full marketplace facilitator arrangement. Etsy and Amazon, as true third-party marketplaces, are more likely to be covered by marketplace facilitator laws for sales made specifically through their platform, shifting the collection obligation onto them.

The practical, actionable step for any seller is not to assume either way — check each platform's current, specific documentation on how it handles sales tax collection for your specific seller setup, and use a dedicated nexus-tracking tool or resource (like TaxJar, cited as a primary source for this article, or Avalara — both repeatedly cited by sellers and SaaS founders as tools used to track nexus and manage remittance) to check your own current nexus footprint directly, since automatic collection on one channel doesn't automatically mean you're covered everywhere you sell ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus); [hn.algolia.com](https://hn.algolia.com/api/v1/search?query=sales%20tax%20nexus)).

## Practical examples

**A small Etsy jewelry shop scaling past $100k in a single state.** If most sales flow through Etsy and Etsy's marketplace-facilitator arrangement handles collection for those in-marketplace sales, the seller's direct compliance burden may be lighter than a similarly-sized seller running their own Shopify store — but the seller should still verify this is actually the case for their specific situation, and check whether any non-Etsy sales channels exist that wouldn't be covered.

**An Amazon FBA seller whose inventory gets distributed to warehouses in five states.** Even without ever visiting those states, opening an office there, or hiring anyone there, this seller may have physical nexus in each of those five states purely because their inventory is physically stored there — a real, documented trap distinct from economic nexus entirely.

**Illustrative example (hypothetical, for clarity).** Imagine a small candle-making business selling primarily through its own Shopify store, doing $80,000 in California sales but with 15,000 individual low-dollar transactions. Since California's economic nexus threshold is $500,000 with no separate transaction-count test cited in the sources above, this seller would not trigger California economic nexus on revenue alone at that sales level — illustrating why understanding your specific state's exact threshold structure (dollar-only vs. combined dollar-and-transaction) matters more than a rough revenue estimate.

## Data and evidence

- As of 2026-08-04, 46 states publish a sales-tax economic nexus dollar threshold for remote sellers; 41 use the common $100,000 threshold, 2 use $250,000 (Alabama, Mississippi), and 3 use $500,000 (California, Texas, New York) ([taxcloud.com](https://taxcloud.com/blog/sales-tax-nexus-by-state/); [numeral.com](https://www.numeral.com/blog/economic-nexus)).
- 18 jurisdictions still maintain a 200-transaction test; Alaska repealed its 200-transaction threshold effective January 1, 2025, and Illinois removed its 200-transaction threshold effective January 1, 2026 ([taxcloud.com](https://taxcloud.com/blog/sales-tax-nexus-by-state/)).
- New York and Connecticut use combined "AND" tests: New York requires $500,000 AND 100 transactions; Connecticut requires $100,000 AND 200 transactions ([nexusbystate.com](https://nexusbystate.com/)).
- Economic nexus exists because of the 2018 US Supreme Court ruling in *South Dakota v. Wayfair, Inc.*, which removed the prior physical-presence-only requirement ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus)).
- Inventory storage, including via third-party fulfillment centers like Amazon FBA, is explicitly listed among nexus-creating activities ([taxjar.com/sales-tax/nexus](https://www.taxjar.com/sales-tax/nexus)).
- A dedicated "Ask HN" thread on handling VAT/sales tax accounting as a B2C SaaS drew 97 points and 86 comments — real, substantial evidence of how heavily-discussed and painful this issue is for small SaaS founders specifically ([hn.algolia.com](https://hn.algolia.com/api/v1/search?query=sales%20tax%20nexus)).
- A separate HN post titled "Selling a B2B SaaS product in the U.S as a U.K company – A tax nightmare?" reflects real, documented founder pain around cross-border US sales tax compliance specifically ([hn.algolia.com](https://hn.algolia.com/api/v1/search?query=sales%20tax%20nexus)).
- A New Jersey bill was proposed to eliminate the transaction-count nexus requirement for sales and corporate business tax, reflecting active legislative efforts to simplify nexus rules in at least some states ([hn.algolia.com](https://hn.algolia.com/api/v1/search?query=sales%20tax%20nexus)).

## Comparisons

**Economic nexus vs. physical nexus.** Physical nexus requires a tangible presence (office, staff, inventory) in a state; economic nexus requires no physical presence at all and is triggered purely by crossing a state-set sales threshold. A seller can have either, both, or neither in any given state, and each must be checked independently.

**TaxJar vs. Avalara.** Both are repeatedly cited as tools small sellers and SaaS founders actually use to track nexus exposure and manage tax remittance across states; the available sourced material doesn't include a detailed, independently verified feature-by-feature comparison between the two — evidence not sufficiently verified beyond both being commonly cited as real, in-use tools for this purpose.

**Origin-based vs. destination-based taxation.** States differ on this basis, which changes how a remote seller calculates the tax rate to charge: origin-based taxation applies the seller's location's rate, while destination-based taxation applies the buyer's location's rate — a distinction that matters once nexus is established in a state and you need to actually calculate the correct tax rate to charge.

## Real-world use cases

- **Etsy and Amazon marketplace sellers** relying partly on marketplace facilitator laws to handle collection for in-marketplace sales, while still needing to track their own nexus footprint for any sales made through other channels.
- **Shopify store owners** who need to actively configure and manage tax collection themselves across every state where they've established nexus, rather than assuming platform-level automation covers this by default.
- **Amazon FBA sellers** specifically needing to track which states their inventory has been distributed to, since this can create physical nexus independent of and in addition to economic nexus from sales volume.
- **Small SaaS founders selling B2C or B2B software to US customers**, who face the same underlying nexus concepts applied to digital products and services, with the added complication of cross-border compliance if the company itself isn't US-based.

## Common mistakes

- **Assuming nexus is a single national rule rather than 46+ independent state-specific determinations.** Each state's threshold, transaction-count rule, and definitions are set independently.
- **Assuming a single revenue figure ("once I hit $X total") determines nexus everywhere.** Nexus thresholds are evaluated per state, against that state's specific dollar and/or transaction-count threshold — not against your total nationwide revenue.
- **Not accounting for Amazon FBA inventory distribution as a source of physical nexus.** Sellers can develop nexus in states they never chose to have a presence in, simply because of Amazon's fulfillment logistics.
- **Assuming a marketplace's tax collection covers all of your sales.** Marketplace facilitator laws typically cover sales made through that specific marketplace — not necessarily sales made through your own website or other channels.
- **Ignoring remote-employee locations as a potential nexus trigger.** Hiring a remote employee in a new state is a real, actively discussed nexus concern, not just an inventory or sales-volume issue.
- **Not re-checking thresholds after a state changes its rules.** States actively update these rules — Alaska and Illinois both recently removed their transaction-count tests — so a rule you checked a year ago may no longer be current.
...
**2. What's the difference between physical nexus and economic nexus?**
...
**3. Does storing inventory in an Amazon FBA warehouse create nexus?**
...
**6. How do small SaaS founders actually handle sales tax day to day?**
...
**7. Does hiring a remote employee in another state create nexus there?**
...
**11. What tools do small businesses actually use to manage multi-state sales tax?**
TaxJar and Avalara are the platforms most commonly cited by sellers and SaaS founders for nexus tracking and remittance.

**12. Are there active efforts to simplify nexus rules?**
...
**13. Does origin-based vs. destination-based taxation matter for remote sellers?**
...
**14. What is economic nexus, specifically?**
...
**15. What is the most common economic nexus dollar threshold across states?**
$100,000 in sales, used by 41 of the 46 states that publish a threshold as of the available 2026 data.

**16. Which states use a higher-than-$100,000 economic nexus threshold?**
Alabama and Mississippi use $250,000; California, Texas, and New York use $500,000.

**17. Which states still use a 200-transaction test?**
...
**18. Which states have removed their 200-transaction threshold recently?**
...
**19. What is South Dakota v. Wayfair, and why does it matter?**
...
**20. Do New York and Connecticut use a different kind of nexus test than most states?**
...
**21. How do I register for a sales tax permit in a state where I have nexus?**
...
**22. How do I determine if I have sales tax nexus in a specific state?**
...
**23. How do I calculate economic nexus thresholds by state?**
...
**24. How do I know if my Amazon FBA inventory has created nexus in a new state?**
...
**25. How do I know if a marketplace is collecting sales tax on my behalf?**
...
**26. How do I handle sales tax if I sell through both my own website and a marketplace?**
...
**27. How do I set up sales tax collection on Shopify?**
...
**28. How do I track sales tax nexus across all 50 states without manual spreadsheets?**
...
**29. How do I handle sales tax nexus as a remote-first company with employees in multiple states?**
...
**30. How do I know when to hire a tax professional versus handling nexus tracking myself?**
...
**31. Economic nexus vs. physical nexus — which is more relevant for a pure online seller?**
...
**32. TaxJar vs. Avalara — which should a small seller use?**
...
**33. Origin-based vs. destination-based taxation — which states use which?**
...
**34. Does Shopify handle sales tax automatically, or do I need to configure it myself?**
...
**35. Does Etsy collect sales tax automatically for sellers?**
...
**36. I just found out I have nexus in a state I've been selling into for years without collecting tax — what do I do?**
...
**37. My FBA inventory got moved to a new state without my choosing it — do I now have nexus there?**
...
**38. I sell through both Etsy and my own Shopify store — am I double-covered for tax collection?**
...
**39. I hired a remote employee in a new state — did that just create sales tax nexus there?**
...
**40. My sales in one state just crossed $100,000 — do I need to register immediately?**
...
**41. Should I use a sales tax automation tool, or can I track this manually with spreadsheets?**
...
**42. Is it worth paying for TaxJar or Avalara for a small, single-state seller?**
...
**43. Should I hire a sales tax professional or handle nexus compliance myself?**
...
**44. What does it cost to get help with multi-state sales tax nexus compliance?**
...
**45. Is a non-US company selling into the US at higher tax-compliance risk than a US-based seller?**
...
**46. What's the best invoicing tool for a small seller who needs to add sales tax correctly?**
The [Invoice Generator](/business/invoice-generator) on tools.scult.in is a straightforward way to produce clean invoices; you'll still need to determine the correct tax rate and whether tax applies for each specific sale based on your actual nexus footprint.

**47. Should an ecommerce business budget for ongoing nexus monitoring as a fixed cost?**
...
**48. Is it worth consulting a tax professional before scaling sales in a new state?**
...
**49. Should a small seller worry about sales tax nexus if they're only selling in their home state right now?**
...
**50. What's the single most important first step for a small online seller trying to understand their sales tax nexus exposure?**
