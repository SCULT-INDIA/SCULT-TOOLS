...
export const meta: BlogPost = {
  ...
  pillar: 'playbook',
  title:
    'Robots.txt AI Crawler Directives: What GPTBot, ClaudeBot & PerplexityBot Actually Respect',
  ...
  description:
    "A practical guide to which AI crawlers honor robots.txt, which don't, and how to write directives that separate AI training from AI search.",
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        [
          '– ',
          { text: 'Training crawler', bold: true },
          ' — GPTBot (OpenAI), ClaudeBot (Anthropic), Google-Extended (Google), CCBot (Common Crawl, whose dataset several labs train on), Applebot-Extended (Apple). These systematically crawl and store content to build or fine-tune a model.',
        ],
        [
          '– ',
          { text: 'Search/retrieval crawler', bold: true },
          ' — OAI-SearchBot (OpenAI), Claude-SearchBot (Anthropic), PerplexityBot (Perplexity). These build the index that powers "search" features inside the respective chat product, closer in spirit to Googlebot than to a training scraper.',
        ],
        [
          '– ',
          { text: 'User-triggered fetch agent', bold: true },
          " — ChatGPT-User (OpenAI), Claude-User (Anthropic), Perplexity-User (Perplexity). These fire only when a live user's question requires fetching a specific page in real time, not on a systematic crawl schedule.",
        ],
        [
    ...
    },
    {
      ...
      body: [
        [
          { text: 'Robots.txt vs. Cloudflare-style bot blocking.', bold: true },
          " Robots.txt is free, universal, and machine-readable by any crawler that chooses to check it — but it has zero enforcement power. Cloudflare's blog post explaining its one-click AI bot blocking feature makes this exact case: it built network-layer blocking, later split into three separate controls for search, training, and agent crawlers, specifically because robots.txt alone couldn't stop crawlers that decide not to honor it (blog.cloudflare.com/declaring-your-aindependence-block-ai-bots-scrapers-and-crawlers-with-a-single-click/). The two approaches are complementary, not competing: robots.txt as the polite, documented request; a CDN/WAF layer as the actual enforcement backstop for anything that ignores the request.",
        ],
        [
          { text: 'GPTBot vs. ChatGPT-User vs. OAI-SearchBot.', bold: true },
          " All three are OpenAI's, but only GPTBot is a systematic training crawl; the other two exist to serve a live user's request, with ChatGPT-User's robots.txt applicability explicitly caveated by OpenAI itself.",
        ],
        [
          { text: 'ClaudeBot vs. Claude-User vs. Claude-SearchBot.', bold: true },
          " Structurally identical split to OpenAI's, except Anthropic documents all three as robots.txt-compliant without OpenAI's caveat about user-triggered fetches, and additionally supports `Crawl-delay`.",
        ],
        [
          { text: 'Google-Extended vs. Googlebot.', bold: true },
          " Not two crawlers at all — one crawler (Googlebot) checking one extra permission token (Google-Extended) that governs a completely separate downstream use (Gemini training) from the one Googlebot's crawl itself feeds (Search indexing).",
        ],
      ],
    },
    {
      ...
      body: [
        [
          '– ',
          { text: 'A local service business', bold: true },
          ' wants to appear when someone asks an AI assistant "best plumber in [city]." Blocking GPTBot/ClaudeBot/Google-Extended doesn\'t hurt this goal at all, since those control training, not the retrieval that answers a live local query; allowing OAI-SearchBot, Claude-SearchBot, and PerplexityBot is what actually matters here.',
        ],
        [
          '– ',
          { text: 'A subscription content publisher', bold: true },
          ' with a hard content-licensing stance blocks every AI crawler category, accepting the trade-off that its content becomes invisible to AI-generated answers entirely, in exchange for stopping bulk reuse of paywalled material by crawlers that do respect the directive.',
        ],
        [
          '– ',
          { text: 'An open-source documentation site', bold: true },
          ' actively wants AI training crawlers in, on the theory that better-trained models producing accurate answers about its API is a net win for adoption — the opposite configuration from the first two examples, and equally valid depending on business goals.',
        ],
        [
          '– ',
          { text: 'An enterprise with existing Cloudflare infrastructure', bold: true },
          " layers Cloudflare's dedicated AI bot controls on top of robots.txt specifically to catch Perplexity-style agents that may not honor the polite request, rather than relying on robots.txt as the sole line of defense.",
        ],
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        [
          'Before finalizing any AI-crawler robots.txt strategy, run your domain through the ',
          { text: 'AI Visibility Checker', href: '/geo/ai-visibility-checker' },
          " to see whether your current configuration and content are actually surfacing in AI-generated answers — the directive is only half the picture; the other half is whether it's producing the visibility outcome you intended.",
        ],
        [
  ...
  faq: [
    ...
    {
      question:
        'Why do so many training crawlers also have a "search" or "user" counterpart?',
      answer: [
    ...
    {
      question:
        'Why did Cloudflare build one-click AI bot blocking instead of relying on robots.txt?',
      answer: [
    ...
    {
      question:
        "How do I opt out of Apple's AI training without leaving Siri/Spotlight search?",
      answer: [
    ...
    {
      question:
        'How do I check whether my CDN is silently blocking a crawler my robots.txt allows?',
      answer: [
    ...
    {
      question:
        'If I block training crawlers, does my content stop appearing in AI chat answers entirely?',
      answer: [
    ...
    {
      question:
        'Can a robots.txt rule apply retroactively to content already used in training?',
      answer: [
    ...
    {
      question:
        'Is blocking AI crawlers via robots.txt as strong as blocking via a paywall or login wall?',
      answer: [
    ...
    {
      question:
        'My robots.txt says "allow" but I still don\'t see AI referral traffic — why?',
      answer: [
    ...
    {
      question:
        "I added a Disallow rule for GPTBot but I'm still seeing it in my server logs — why?",
      answer: [
    ...
    {
      question:
        "My CDN shows AI crawlers being blocked even though robots.txt allows them — what's happening?",
      answer: [
    ...
    {
      question:
        "I blocked Google-Extended and now I'm worried about my Search rankings — should I be?",
      answer: [
    ...
    {
      question:
        'Is it worth paying for a dedicated AI-crawler-blocking service instead of just editing robots.txt myself?',
      answer: [
    ...
    {
      question:
        'Should a small business bother with AI crawler directives at all, or is this overkill?',
      answer: [
    ...
    {
      question:
        'How do I know if my current robots.txt configuration is actually helping or hurting my AI visibility?',
      answer: [
    ...
    {
      question:
        "Where should I start if I've never touched robots.txt for AI crawlers before?",
      answer: [
