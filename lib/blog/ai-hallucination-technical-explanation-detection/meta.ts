...
export const meta: BlogPost = {
  ...
  description:
    'A technical breakdown of what counts as an AI hallucination, why it happens, and the real detection methods teams use in production — groundedness checks, LLM-as-judge, and multi-model consensus.',
  ...
  sections: [
    {
      ...
      body: [
        [
          'Casually, "hallucination" gets used for any wrong AI answer. Technically, the term is narrower and more useful than that. A model can be wrong because a fact is genuinely obscure, disputed, or outdated in its training data — that\'s a knowledge gap, not a hallucination in the strict sense. A hallucination, properly defined, is when the model ',
          {
            text: 'generates a claim that has no support in its training data or provided context, and states it with the same confident, fluent register it would use for a verified fact.',
            bold: true,
          },
          " The output looks identical whether it's right or fabricated, which is exactly why it's dangerous.",
        ],
        [
        ...
        [
          'This is also why "hallucination" in a retrieval-augmented generation (RAG) system means something more specific still: it\'s when the model\'s answer is not supported by the documents it retrieved and was supposed to answer from. In a RAG pipeline, you have ground truth sitting right there in the context window — so a hallucination in this setting is a ',
          { text: 'grounding failure', bold: true },
          ", not just a factual error against the wider world. This distinction matters because it changes the detection strategy: instead of needing an external fact-checking system, you can check the model's claims against the specific text it was given.",
        ],
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        ...
        [
          '– ',
          { text: 'Attributable', bold: true },
          ' — the claim is directly supported by the source context the model was given. This is the "safe" bucket.',
        ],
        [
          '– ',
          { text: 'Extrapolatory', bold: true },
          " — the claim goes beyond what the source explicitly says, but isn't necessarily false; it's an inference the model made that can't be verified either way from the provided material.",
        ],
        [
          '– ',
          { text: 'Contradictory', bold: true },
          ' — the claim directly conflicts with something stated in the source context. This is the clearest, most dangerous form of hallucination in a RAG setting, because it means the model actively misrepresented the material it was given.',
        ],
        [
    ...
    },
    {
      ...
      body: [
        ...
        [
          'One open-source framework surfaced in these discussions goes further and traces the ',
          { text: 'full execution path', bold: true },
    ...
    },
    {
      ...
      body: [
        ...
        [
          '– Published academic work tracking this leaderboard finds that reasoning-oriented model configurations do not uniformly reduce hallucination on grounded tasks, and in some measured cases increase it relative to non-reasoning configurations on the same benchmark (',
          {
            text: 'arxiv.org/pdf/2505.24630',
            href: 'https://arxiv.org/pdf/2505.24630',
            external: true,
        ...
        [
          '– Specific cross-model hallucination-rate percentages circulating in mid-2026 industry blog coverage vary widely between sources and reference inconsistent model-version naming; given this inconsistency, exact percentage comparisons between specific model versions should be treated as ',
          { text: 'evidence not sufficiently verified', bold: true },
          ' rather than cited as settled fact — check the live Vectara leaderboard directly for current, reproducible numbers rather than relying on secondary blog summaries.',
        ],
        [
          '– The 2024 academic paper "The Hallucinations Leaderboard" documents an open, standardized effort to measure hallucination specifically (as distinct from general capability) across both closed- and open-source models, reflecting that the field treats this as a separate, dedicated measurement axis (',
          {
            text: 'arxiv.org/pdf/2404.05904',
            href: 'https://arxiv.org/pdf/2404.05904',
            external: true,
          },
          ').',
        ],
    ...
    },
    {
      ...
      body: [
        [
          { text: 'Groundedness checking vs. general fact-checking.', bold: true },
    ...
    },
    {
      ...
      body: [
        [
          '– ',
          { text: 'RAG-based internal knowledge assistants', bold: true },
          ' run groundedness checks as a gating step before an answer reaches an employee, rejecting or flagging any response containing claims not traceable to the indexed documents.',
        ],
        [
          '– ',
          { text: 'Customer-facing support bots', bold: true },
          " use LLM-as-judge scoring on every response before it's sent, specifically checking for policy claims (refunds, pricing, legal terms) that aren't backed by the retrieved knowledge base.",
        ],
        [
          '– ',
          { text: 'Coding agents and code-review tooling', bold: true },
          ' face a documented hallucination risk around invented APIs and function signatures, which is a distinct sub-problem from prose hallucination and generally requires execution-based or static-analysis-based verification rather than text-level groundedness checks alone.',
        ],
        [
          '– ',
          { text: 'Production monitoring pipelines', bold: true },
          ' that trace full execution paths via decorators (as described in the open-source framework referenced above) treat hallucination detection as an ongoing regression-test surface, re-running fixed quality checks every time a prompt or pipeline logic changes.',
        ],
      ],
    },
    {
      ...
      body: [
        [
          '– ',
          {
            text: 'Treating "the model sounds confident" as evidence of correctness.',
            bold: true,
          },
          " Confidence and fluency are properties of language generation, not properties of truth — they're exactly as present in a hallucinated claim as in a correct one.",
        ],
        [
          '– ',
          { text: 'Grading answers as fully true or fully false.', bold: true },
          ' Claim-level decomposition (FactScore-style) catches partial hallucination that whole-answer grading misses entirely.',
        ],
        [
          '– ',
          {
            text: 'Using the same model to generate and judge its own output with no independent grounding check.',
            bold: true,
          },
          " This risks the judge sharing the generator's blind spots, especially on rare or specialized topics.",
        ],
        [
          '– ',
          {
            text: 'Only checking hallucination at launch, not continuously.',
            bold: true,
          },
          ' Model updates, prompt changes, and shifts in retrieved-document quality can all reintroduce hallucination that passed a one-time evaluation.',
        ],
        [
          '– ',
          {
            text: "Ignoring extrapolatory claims because they aren't outright contradictions.",
            bold: true,
          },
          ' Extrapolatory claims are unverifiable, not verified — treating them as safe by default is a common source of quietly wrong answers reaching users.',
        ],
        [
          '– ',
          {
            text: 'Skipping groundedness checks in non-RAG generation tasks',
            bold: true,
          },
          ' (like open-ended code generation) on the assumption that hallucination detection is only a RAG problem.',
        ],
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        [
          "If you're building or evaluating agent-based systems where hallucination risk matters, the ",
          { text: 'Claude prompt library', href: '/prompts/claude' },
          " on tools.scult.in has practical prompt patterns for structuring instructions and verification steps around a model's output. The ",
          { text: 'AI Agents & RAG prompt collection', href: '/prompts/ai-engineering' },
          " is also directly relevant if you're mid-build on a retrieval pipeline or support bot and need prompt-level guardrails against exactly this failure mode.",
        ],
        [
          "Teams building production RAG pipelines, agents, or internal AI tools that need this kind of evaluation infrastructure designed and wired in correctly are exactly the kind of project SCULT's ",
          {
  ...
  faq: [
    ...
    },
    {
      ...
      answer: [
        "Not straightforwardly. Some published research on reasoning-model configurations found they don't automatically hallucinate less on grounded tasks, and can hallucinate more, since longer reasoning chains create more opportunities to drift from the source (",
        {
          text: 'arxiv.org/pdf/2505.24630',
          href: 'https://arxiv.org/pdf/2505.24630',
          external: true,
    ...
    },
    {
      ...
      answer: [
        'A widely cited, actively maintained open benchmark measuring hallucination and refusal rates on grounded summarization tasks using fixed datasets, enabling reproducible comparison across model releases (',
        {
          text: 'vectara.com/blog',
          href: 'https://www.vectara.com/blog/introducing-the-next-generation-of-vectaras-hallucination-leaderboard',
          external: true,
    ...
    },
    {
      ...
      answer: [
        'Not reliably, per published leaderboard research — reasoning-heavy configurations have been measured hallucinating more on some grounded tasks, since each additional inferential step is a chance to drift from the source (',
        {
          text: 'arxiv.org/pdf/2505.24630',
          href: 'https://arxiv.org/pdf/2505.24630',
          external: true,
